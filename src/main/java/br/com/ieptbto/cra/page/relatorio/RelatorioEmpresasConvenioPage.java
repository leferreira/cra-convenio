...
public class RelatorioEmpresasConvenioPage extends BasePage<Instituicao>{

	@SpringBean
	FiliadoMediator filiadoMediador;
	@SpringBean
	...
	public RelatorioEmpresasConvenioPage() {
		...
		formPeriodo.add(botaoGerarRelatorio());
		add(formPeriodo);
		add(listaEmpresasTitulos());
	}
		...
		return new Link<Arquivo>("gerarRelatorio"){
			...
			public void onClick() {
				try {
					...
					JasperReport jasperReport = JasperCompileManager.compileReport(getClass().getResourceAsStream("RelatorioEmpresasConveniosSinteticos.jrxml"));
					...
					getRequestCycle().scheduleRequestHandlerAfterCurrent(
					        new ResourceStreamRequestHandler(resourceStream, pdf.getName()));
	...
	private ListView<Filiado> listaEmpresasTitulos() {
