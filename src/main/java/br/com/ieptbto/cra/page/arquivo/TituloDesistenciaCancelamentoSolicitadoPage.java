...
public class TituloDesistenciaCancelamentoSolicitadoPage extends BasePage<SolicitacaoDesistenciaCancelamentoConvenio> {
	...
	Form<SolicitacaoDesistenciaCancelamentoConvenio> form = new Form<SolicitacaoDesistenciaCancelamentoConvenio>("form", getModel()) {
		...
		try {
			...
			if (getTituloRemessa().getConfirmacao() != null) {
				...
				if (!numeroProtocolo.trim().equals(StringUtils.EMPTY)
					|| !numeroProtocolo.trim().equals(0)) {
			...
			// if
			// (getTituloFiliado().getSolicitacaoDesistenciaCancelamento()
			// != null) {
			// throw new InfraException("O título já contém um
			// pedido de Desistência/Cancelamento !");
			// }
		    }
	...
	return new Label("dataEmissaoTitulo", new Model<String>(DataUtil.localDateToString(new LocalDate(getTituloFiliado().getDataEmissao()))));
    }

    private Label dataVencimentoTitulo() {
	return new Label("dataVencimentoTitulo", new Model<String>(DataUtil.localDateToString(new LocalDate(getTituloFiliado().getDataVencimento()))));
    }

    public Label valorTitulo() {
	Label textField = new Label("valorTitulo", new Model<String>("R$ "
		+ getTituloFiliado().getValorTitulo().toString()));
	return textField;
    }

    public Label saldoTitulo() {
	Label textField = new Label("saldoTitulo", new Model<String>("R$ "
		+ getTituloFiliado().getValorSaldoTitulo().toString()));
	return textField;
    }
